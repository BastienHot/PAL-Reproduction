...
for name in datatype:
    # CHANGE: updated path to use ./data/ directory (was: Persona_chat/personachat/)
    # Download PersonaChat data and place {train,valid,test}_both_original.txt in ./data/
    file_path = f'./data/{name}_both_original.txt'
    ...
    # CHANGE: output to ./data/ to match train_bart.py's expected data dir (was: ./{name}.csv)
    tmp_df.to_csv(f"./data/{name}.csv")
